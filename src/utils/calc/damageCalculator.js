...
export class DamageCalculator {
  ...
  getAttackType() {
    ...
    if (this.attacker instanceof Psykeeper) {
  ...
  getHitRate() {
    ...
    return Utils.clamp(
      hitProb + this.attacker.getExtraParam('hitrate', true),
      20,
      96,
    )
  }
  ...
  getDamageMultiplier() {
    ...
    if (this.isSkill) {
      ...
      switch (this.skill.id) {
        case 5162: // Vital Stab
          ...
          factor *=
            1 -
            skillLevelProp.probability / 100.0 +
            2 * (skillLevelProp.probability / 100.0)
          break
    ...
    if (
      this.attacker instanceof Knight &&
      this.weapon.triggerSkillProbability != undefined
    ) {
      factor *= this.getAverageChanceMultiplier(
        this.weapon.triggerSkillProbability,
        2.0,
      )
    }
    ...
    if (delta > 0) {
      const reduceFactor = [
        1.0, 1.0, 0.98, 0.95, 0.91, 0.87, 0.81, 0.75, 0.67, 0.59, 0.51, 0.42,
        0.32, 0.22, 0.12, 0.01,
      ]
  ...
  applyGenericDefense(attack) {
    ...
    defense = Math.floor(
      (defense * this.defenseFactor) / Moverutils.ATTACK_ELEMENT_FACTOR,
    )
    ...
    if (damage > 0) {
      ...
      const criticalBonus = Math.max(
        0.1,
        1.0 + this.attacker.getExtraParam('criticaldamage', true) / 100.0,
      )
      const criticalDamage = Math.floor(criticalFactor * criticalBonus * damage)

      damage = Math.floor(
        Utils.lerp(damage, criticalDamage, criticalChance / 100),
      )
  ...
  getBlockFactor() {
    ...
    let blockRate = Math.floor(
      (this.defender.parry - this.defender.level) * 0.5,
    )
  ...
  applyElementDefense(attack) {
    ...
    const attackerElement =
      skillElement != Moverutils.Elements.none ? skillElement : weaponElement
    ...
    if (skillElement != Moverutils.Elements.none) {
      ...
      if (
        (weaponElement == Moverutils.Elements.water &&
          skillElement == Moverutils.Elements.fire) ||
        (weaponElement == Moverutils.Elements.electricity &&
          skillElement == Moverutils.Elements.water) ||
        (weaponElement == Moverutils.Elements.earth &&
          skillElement == Moverutils.Elements.electricity) ||
        (weaponElement == Moverutils.Elements.wind &&
          skillElement == Moverutils.Elements.earth) ||
        (weaponElement == Moverutils.Elements.fire &&
          skillElement == Moverutils.Elements.wind)
      ) {
  ...
  applyMagicSkillDefense(attack) {
    ...
    if (this.attackContext == Moverutils.AttackContext.AC_PVP) {
      attack -=
        (attack * this.defender.getExtraParam('magicDefense', true)) / 100
    }
  ...
  applyDefenseParryCritical(attack) {
    ...
    if (!this.isSkill) {
      ...
      return Math.floor(
        damage * (1 - criticalChance / 100.0) +
          criticalDamage * (criticalChance / 100.0),
      )
    }
  ...
  computeGenericDefense() {
    ...
    let defense = Math.floor(
      level * levelScale +
        (this.defender.sta * statScale + (this.defender.sta - 14) * jobFactor) *
          staFactor -
        4,
    )
    equipmentDefense /= 4
  ...
  getMeleeSkillPower() {
    ...
    powerMin =
      ((weaponAttack[0] +
        (levelProp.minAttack + this.weapon.additionalSkillDamage) * 5 +
        referStat -
        20) *
        (16 + skillLevel)) /
      13
    powerMax =
      ((weaponAttack[1] +
        (levelProp.maxAttack + this.weapon.additionalSkillDamage) * 5 +
        referStat -
        20) *
        (16 + skillLevel)) /
      13

    const weaponDamage = this.attacker.getExtraParam(
      `${this.weapon.subcategory}attack`,
    )

    powerMin =
      this.attacker.getExtraParam('damage') +
      Math.floor(powerMin) +
      weaponDamage
    powerMax =
      this.attacker.getExtraParam('damage') +
  ...
  getMagicSkillPower() {
    ...
    attack +=
      (attack * this.attacker.getExtraParam('magicattack', true)) / 100.0
  ...
  getHitPower() {
    ...
    attack = Math.floor(
      (attack * attackFactor) / Moverutils.ATTACK_ELEMENT_FACTOR,
    )
  ...
  getHitMinMax() {
    ...
    const plus =
      this.getWeaponAttack(this.weapon.subcategory) +
      this.attacker.getExtraParam('damage')
    min += plus
    max += plus
    ...
    if (this.attacker.mainhandUpgrade > 0) {
      const value = Math.floor(this.attacker.mainhandUpgrade ** 1.5)
      min += value
      max += value
    }
  ...
  getDamagePropertyFactor() {
    ...
    if (this.attacker.mainhandElement != Moverutils.Elements.none) {
      ...
      attackLevel =
        this.attacker.mainhandElementUpgrade +
    ...
    if (
      attackType == Moverutils.Elements.none &&
      defenseType == Moverutils.Elements.none
    ) {
      return attackFactor
    }
  ...
  getAttackMultiplier() {
    ...
    // Upcut stone
    if (
      this.attacker.activePremiumItems.find(
        (buff) => buff.id == 8691 && buff.enabled,
      )
    ) {
      factor *= 1.2
    }
  ...
  getWeaponAttackPower() {
    ...
    if (upgrade > 0) {
      add = Math.floor(upgrade ** 1.5)
    }

    f = this.getWeaponMultiplier()

    const power = [this.weapon.minAttack, this.weapon.maxAttack]
  ...
  getStatScale(level, levelProp, isPVP) {
    let total = 0
    const spellContext = isPVP
      ? Moverutils.AttackContext.AC_PVP
      : Moverutils.AttackContext.AC_PVE
  ...
  getDamagePerSecond() {
    ...
    if (this.attackType == Moverutils.AttackType.AUTO_ATTACK) {
      ...
      hitsPerSecond =
        this.attacker.constants.hps * attackSpeed * (hitRate / 100)
      ...
      const cooldown =
        skillLevelProp.cooldown != undefined ? skillLevelProp.cooldown : 0
