    ...
    if (user && user.uid) {
      ...
      const updatedCharacters =
        characterData.length < 8
          ? [...characterData, ...Array(8 - characterData.length).fill(null)]
          : characterData
      await setUserChars(updatedCharacters)
    }
    return userChars
  }
              ...
              <div className="flex flex-1 flex-col p-8">
                ...
                <dl className="mt-1 flex flex-grow flex-col justify-between">
                  <dd className="text-sm text-gray-100">
                    <span className="font-bold">{userChar[id].action}</span>
                    <span>
                      {' '}
                      since {getLastOnline(userChar[id].lastOnline)} ago
                    </span>
                  </dd>
                  <dd className="mt-3">
